...
wsim = sigma_w*np.fabs(np.random.randn(Nsim, Nw))
...
for t in range(Nsim):
    # Measure.
    ysim[t] = np.around(model.H(xsim_ltv[t,:]) + vsim[t,:], decimals=0)
...
x_0 = x0 + 3.0*sigma_p*np.random.randn(Nx)
...
for t in range(Nsim):
    ...
    sol = tools.nmhe_ltv(f_casadi, h_casadi, usim[tmin:tmax-1,:], ysim[tmin:tmax,:], l, N,
                   lx=lx, x0bar=x_0, P0=linalg.inv(P), Delta=Delta, ltv_guess=ltv_guess, returnSolver=False)
